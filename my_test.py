...
def eval_graph(path, train_data_loader, test_data,):
    dn = osp.basename(path)
    if 'graph.p' not in os.listdir(path):
        return
    output_fn = osp.join(path, 'eval_results.json')
    ...
    optim = torch.optim.Adam(grounder.parameters(), lr=3e-4, eps=1e-5)
    ...
    with open(output_fn, 'w') as f:
        json.dump({
            'task_id': task_id, 'program': program, 'symbol_acc': sym_acc, 'program_acc': prog_acc, 'loss': loss.item(),
...
def get_data():
    ...
    return data_loader, test_dataset[:]
